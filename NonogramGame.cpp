...
const size_t MAX_POSSIBLE_LEVEL_OF_THE_GAME = 5;
const size_t MIN_POSSIBLE_LEVEL_OF_THE_GAME = 1;

unsigned LengthOfCharArr(const char* arr)
{
	int len = 0;
	while (*arr)
	{
...
{
	cout << "Please, choose the level you  want to play(between 1 and 5): ";
	cin >> level;
	while (level < MIN_POSSIBLE_LEVEL_OF_THE_GAME || level > MAX_POSSIBLE_LEVEL_OF_THE_GAME)
	{
		cout << "Inccorect choice! Please, try again, you can choose between 1 and 5!";
		cin >> level;
	}
}
...
void SelectAndLoadLevel(size_t level, string& matrix, size_t sizeOfMatrix)
{
	...
	case 4:
		if (randomOptionSelector == 0)
		{
			currentFileOfUsersChoice.open("level_4_v1.txt");
		}
		else
		{
			currentFileOfUsersChoice.open("level_4_v2.txt");
		}
		countOfPossibleMistakesAccordingToTheCurrentLevel = 6;
		break;
	case 5:
		if (randomOptionSelector == 0)
		{
			currentFileOfUsersChoice.open("level_5_v1.txt");
		}
		else
		{
			currentFileOfUsersChoice.open("level_5_v2.txt");
		}
		countOfPossibleMistakesAccordingToTheCurrentLevel = 7;
		break;
	default:
		cout << "The game supports only 5 levels!";
		break;
	}
...
bool isAllNonogramPuzzleSolved(char** puzzle, size_t x_CoordinateOfMatrix, size_t y_CoordinateOfMatrix,
...
void ContinueWithNextLevel(int level)
{
	...
	while (true)
	{
		...
		if (isAllNonogramPuzzleSolved(puzzle, x_Coordinate, y_Coordinate, mistakes, currentUserSymbol, sizeOfPuzzle))
		{
			bool continueAnswer;
